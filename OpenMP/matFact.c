...
#pragma omp parallel for default(none) shared(nUser, nItem, nFeat, B, L, R)
  for (int i = 0; i < nUser; i++)
    for (int j = 0; j < nItem; j++) {
      B[i][j] = 0;
      for (int k = 0; k < nFeat; k++)
...
#pragma omp parallel for default(none) private(B_item, sol_aux, A_aux1) shared(B, solution, nUser, A_user, nItem) schedule(dynamic)
